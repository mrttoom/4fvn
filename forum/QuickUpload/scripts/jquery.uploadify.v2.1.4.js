...
var bbcode='';
var directlink='';
var it4t_parent_window=null;

//--Get Parent Window
if(window.opener!=null)
	it4t_parent_window=window.opener;
else if(window.parent!=null && window.parent!=window)
	it4t_parent_window=window.parent;
			...
			uploadify:function(options) {
				...
				jQuery(this).bind("uploadifyQueueFull", {'action': settings.onQueueFull}, function(event, queueSizeLimit) {
					if (event.data.action(event, queueSizeLimit) !== false) {
						alert('Số lượng file chọn cùng lúc vượt mức cho phép. Website cho phép up cùng lúc tối đa ' + queueSizeLimit + '.');
					}
				});
					...
					jQuery.post(checkScript, postData, function(data) {
						for(var key in data) {
							if (event.data.action(event, data, key) !== false) {
								var replaceFile = confirm("Bạn có muốn thay thế file " + data[key] + "?");
								if (!replaceFile) {
				...
				jQuery(this).bind("uploadifyComplete", {'action': settings.onComplete}, function(event, ID, fileObj, response, data) {
					if (event.data.action(event, ID, fileObj, unescape(response), data) !== false) {
						jQuery("#" + jQuery(this).attr('id') + ID).find('.percentage').text(' - Completed');
							...
							trave=unescape(response);
	
							if(trave.indexOf('loi|')==0)
								loi += trave.substr(4,trave.length)+'\n';
							else
								{
									directlink += trave+'\n';
									bbcode += '[IMG]'+trave+'[/IMG]';
								}
				...
				if (typeof(settings.onAllComplete) == 'function') {
					jQuery(this).bind("uploadifyAllComplete", {'action': settings.onAllComplete}, function(event, data) {
						...
						it4t='<b>BBCode:</b><br><textarea id=\"bbcodearea\" style=\"width:97%;height:40%;\" onclick=\"this.select()\">'+bbcode+'</textarea><b>Direct Link:</b><br><textarea style=\"width:97%;height:40%\" onclick=\"this.select()\">'+directlink+'</textarea>';
						document.getElementById('fileQueue').innerHTML=it4t;
						showup=true;
						if(loi)
							alert('Lỗi gặp phải khi Upload:\n'+loi);
						it4t_parent_window.it4t_InsertText(bbcode);
