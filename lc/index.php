<?php
/***************************************************************************\
| Sypex Dumper               version 2.0.5 Beta                             |
| (c) 2003-2009 zapimir      zapimir@zapimir.net       http://sypex.net/    |
| (c) 2005-2009 BINOVATOR    info@sypex.net                                 |
|---------------------------------------------------------------------------|
|     created: 2003.09.02 19:07              modified: 2009.12.15 23:34     |
|---------------------------------------------------------------------------|
...
class Sypex_Dumper {
	function Sypex_Dumper() {
		...
		$this->name = 'Sypex Dumper 2.0.5 Beta';
	}
	...
	function runRestoreJob($continue = false){
		...
		while($q = sxd_read_sql($this->fh_tmp, $seek, $ei)){	
			...
			if($ex) {
				...
				if(mysql_query($q)) {
					...
					unset($this->rtl);
					trigger_error(mysql_error());exit;
				}
		...
		foreach($this->JOB['todo']['TA'] AS $tab){
			...
			mysql_query("ALTER TABLE `{$tab[1]}` ENABLE KEYS") or die (mysql_error());
			$this->rtl[1] = time();
			$this->rtl[5] = $tab[1];
			fseek($this->fh_rtl, 0);
			fwrite($this->fh_rtl, implode("\t", $this->rtl));
...
function sxd_shutdown(){
	global $SXD;
	if(!empty($SXD->fh_rtl) && !empty($SXD->rtl)) {
		$SXD->rtl[1] = time();
...
<script type="text/javascript" src="load.php?sxd.v205.js"></script>
